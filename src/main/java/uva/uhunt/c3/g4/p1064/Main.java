...
class Process {
    public Output process(final Input input) {
        ...
        do {
            ...
            for (final Packet packet : packets) {
                packet.sequence = messages[packet.messageId].sequence;
            }

            int bufferSize = getBufferSize(packets, messages);
    ...
    private int getBufferSize(final Packet[] packets, final Message[] messages) {
        ...
        for (final Packet packet : packets) {
            ...
            while (!buffer.isEmpty()) {
                final Packet top = buffer.peek();
                if (top.sequence != lastSequence) break;

                final Message message = messages[top.messageId];
                if (!message.isConsumable(top)) break;
...
class Packet {
    public static final Comparator<Packet> ORDER_BY_SEQUENCE_AND_START = Comparator
        .comparingInt((Packet p) -> p.sequence)
